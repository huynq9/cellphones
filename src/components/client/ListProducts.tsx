...
const ListProducts = ({ data }: Props) => {
  return (
    <Link to={`/detail/${data._id}`} className="">
      ...
      <div className="product w-[220px] h-[330px] hover:shadow-xl p-3 hover:scale-110 transform transition duration-500">
        <div className=" ">
          <img
            style={{ objectFit: "cover" }}
            src={data.image ? data.image : `/product1.png`}
            alt=""
            className="mx-auto block w-[170px] h-[170px] hover:scale-110 transform transition duration-500"
          />
        </div>
        <div className="">
